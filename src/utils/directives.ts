...
function zoomDirective(app: App) {
  app.directive("zoom", {
    mounted(el, binding) {
      if (!binding.value) return false;
      const zoomDomBindData = isString(binding.value)
        ? [binding.value, ".el-dialog__body", false, true]
        : binding.value;
      zoomDomBindData[1] = zoomDomBindData[1]
        ? zoomDomBindData[1]
        : ".el-dialog__body";
      zoomDomBindData[2] =
        typeof zoomDomBindData[2] == "undefined" ? false : zoomDomBindData[2];
      zoomDomBindData[3] =
        typeof zoomDomBindData[3] == "undefined" ? true : zoomDomBindData[3];

      nextTick(() => {
        const zoomDom = document.querySelector(
          zoomDomBindData[1],
        ) as HTMLElement; // 实际被缩放的元素
        const zoomDomBox = document.querySelector(
          zoomDomBindData[0],
        ) as HTMLElement; // 动态添加缩放句柄的元素
