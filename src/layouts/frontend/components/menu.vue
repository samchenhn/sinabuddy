  ...
  >
    ...
    <template v-if="memberCenter.state.open">
      <el-sub-menu
        ...
        @click="$attrs.mode == 'vertical' ? '' : router.push({ name: 'user' })"
        v-blur
        index="user-box"
      >
        <template #title>
          <div class="header-user-box">
            <img
              ...
              :class="
                $attrs.mode == 'vertical' ? 'icon-header-user-avatar' : ''
              "
              :src="
                fullUrl(
                  userInfo.avatar
        ...
        <!-- 动态菜单 -->
        <MenuSub
          :menus="memberCenter.state.navUserMenus"
          :show-icon="showIcon"
        />
...
const findMenus = (route: RouteLocationNormalizedLoaded) => {
  ...
  // 会员中心下拉菜单
  const navUserMenuIndex = searchMenuIndex(
    memberCenter.state.navUserMenus,
    route,
  );
