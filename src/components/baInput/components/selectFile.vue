    ...
    >
      <TableHeader
        ...
        :quick-search-placeholder="
          t('Quick search placeholder', { fields: t('utils.Original name') })
        "
      >
  ...
  column: [
    ...
    },
    {
      ...
      formatter: (row: TableRow, column: TableColumn, cellValue: string) => {
        ...
        return (
          parseInt((size / Math.pow(1024, i)).toFixed(i < 2 ? 0 : 2)) * 1 +
          " " +
          ["B", "KB", "MB", "GB", "TB"][i]
        );
      },
...
const getData = () => {
  if (props.type == "image") {
    baTable.table.filter!.search = [
      { field: "mimetype", val: "image", operator: "LIKE" },
    ];
  }
...
const onChoice = () => {
  if (baTable.table.selection?.length) {
    ...
    for (const key in baTable.table.selection) {
      files.push(
        props.returnFullUrl
          ? baTable.table.selection[key].full_url
          : baTable.table.selection[key].url,
      );
    }
    emits("choice", files);
    const elTableRef = tableRef.value?.getRef();
