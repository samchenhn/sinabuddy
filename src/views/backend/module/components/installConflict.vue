  ...
  <div>
    <div class="install-conflict">
      <template v-if="state.common.fileConflict.length > 0">
        ...
        <el-table
          :data="state.common.fileConflict"
          stripe
          border
          :style="{ width: '100%' }"
        >
          ...
          <el-table-column
            prop="oldFile"
            :label="$t('module.Existing files')"
          />
          <el-table-column
            prop="solution"
            width="200"
            :label="$t('module.Treatment scheme')"
            align="center"
          >
            <template #default="scope">
              <el-select v-model="scope.row.solution">
                <el-option
                  :label="$t('module.Backup and overwrite existing files')"
                  value="cover"
                ></el-option>
                <el-option
                  :label="$t('module.Discard new file')"
                  value="discard"
                ></el-option>
              </el-select>
            </template>
      ...
      <template v-if="state.common.dependConflict.length > 0">
        ...
        <el-table
          :data="state.common.dependConflict"
          stripe
          border
          style="width: 100%"
        >
          ...
          <el-table-column
            prop="newDepend"
            :label="$t('module.New dependency')"
          />
          <el-table-column
            prop="oldDepend"
            :label="$t('module.Existing dependencies')"
          />
          <el-table-column
            prop="solution"
            width="200"
          ...
          >
            <template #default="scope">
              <el-select v-model="scope.row.solution">
                <el-option
                  :label="$t('module.Overwrite existing dependencies')"
                  value="cover"
                ></el-option>
                <el-option
                  :label="$t('module.Do not use new dependencies')"
                  value="discard"
                ></el-option>
              </el-select>
            </template>
...
const onSubmitConflictHandle = () => {
  ...
  for (const key in state.common.fileConflict) {
    fileConflict[state.common.fileConflict[key].oldFile] =
      state.common.fileConflict[key]["solution"];
  }
  for (const key in state.common.dependConflict) {
    if (
      typeof dependConflict[state.common.dependConflict[key].env] == "undefined"
    ) {
      dependConflict[state.common.dependConflict[key].env] = {};
    }
    dependConflict[state.common.dependConflict[key].env][
      state.common.dependConflict[key].depend
    ] = state.common.dependConflict[key]["solution"];
  }
