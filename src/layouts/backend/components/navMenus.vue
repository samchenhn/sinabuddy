  ...
  <div class="nav-menus" :class="configStore.layout.layoutMode">
    ...
    >
      <div>
        ...
        <div class="reload-hot-server-content">
          <p>
            ...
            <span
              >【{{ t(`vite.Close type ${hotUpdateState.closeType}`) }}】</span
            >
            <span>{{ t("vite.Reload hot server tips 2") }}</span>
          </p>
          <p>{{ t("vite.Reload hot server tips 3") }}</p>
          <div class="reload-hot-server-buttons">
            <el-button @click="onHotServerOpt('cancel')">{{
              t("vite.Later")
            }}</el-button>
            <el-button @click="onHotServerOpt('reload')" type="primary">{{
              t("vite.Restart hot update")
            }}</el-button>
          </div>
        </div>
      </div>
      <template #reference>
        <div
          class="nav-menu-item"
          :class="state.currentNavMenu == 'reloadHotServer' ? 'hover' : ''"
        >
          <Icon
