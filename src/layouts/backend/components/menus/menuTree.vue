      ...
      >
        <template #title>
          <Icon
            ...
            :name="
              menu.meta?.icon ? menu.meta?.icon : config.layout.menuDefaultIcon
            "
          />
          <span>{{ menu.meta?.title ? menu.meta?.title : $t("noTitle") }}</span>
        </template>
        <MenuTree
          :extends="{ ...props.extends, level: props.extends.level + 1 }"
          :menus="menu.children"
        />
      </el-sub-menu>
    </template>
...
const onClickSubMenu = (menu: RouteRecordRaw) => {
  if (
    props.extends?.position == "horizontal" &&
    props.extends.level <= 1 &&
