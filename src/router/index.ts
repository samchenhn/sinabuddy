...
router.beforeEach((to, from, next) => {
  ...
  if (isAdminApp(to.fullPath)) {
    ...
    // 去除 path 中的 /admin
    const adminPath = to.path.slice(
      to.path.indexOf(adminBaseRoutePath) + adminBaseRoutePath.length,
    );
  ...
  for (const key in loadPath) {
    loadPath[key] = loadPath[key].replaceAll(
      "${lang}",
      config.lang.defaultLang,
    );
    if (loadPath[key] in window.loadLangHandle) {
      window.loadLangHandle[loadPath[key]]().then(
        (res: { default: anyObj }) => {
