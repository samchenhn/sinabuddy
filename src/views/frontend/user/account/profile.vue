<template>
  <div class="user-views">
    <el-card class="user-views-card" shadow="hover">
      <template #header>
        <div class="card-header">
          ...
          <el-button
            @click="router.push({ name: 'account/changePassword' })"
            type="info"
            v-blur
            plain
          >
      ...
      <div class="user-profile">
        ...
        >
          ...
          <FormItem
            ...
            :placeholder="
              $t('Please input field', {
                field: $t('user.account.profile.User name'),
              })
            "
            prop="username"
          />
          <FormItem
            ...
            :placeholder="
              $t('Please input field', {
                field: $t('user.account.profile.User nickname'),
              })
            "
            prop="nickname"
          />
          <el-form-item
            ...
            :label="t('user.account.profile.email')"
          >
            <el-input
              v-model="state.form.email"
              readonly
              :placeholder="
                t('user.account.profile.Operation via right button')
              "
            >
              <template #append>
                <el-button type="primary" @click="onChangeBindInfo('email')">
                  {{
                    state.form.email
                      ? t("user.account.profile.Click Modify")
          ...
          <el-form-item
            ...
            :label="t('user.account.profile.mobile')"
          >
            <el-input
          ...
          />
          <FormItem
            :label="$t('user.account.profile.birthday')"
            type="date"
          ...
          <FormItem
            ...
            :placeholder="
              $t('Please input field', {
                field: $t('user.account.profile.Personal signature'),
              })
            "
    ...
    >
      ...
      >
        <FormItem
          ...
          :placeholder="
            $t('Please input field', {
              field: $t('user.account.profile.password'),
            })
          "
        />
        <el-form-item prop="captcha">
          <template #label>
            <span v-if="state.dialog.type == 'email'">
              ...
              ({{
                t("user.account.profile.accept") +
                t("user.account.profile.mail") +
                "：" +
                userInfo.email
              }})
            </span>
          ...
          <el-row class="w100" :gutter="10">
            <el-col :span="18">
              <el-input
                ...
                :placeholder="
                  t('Please input field', {
                    field: t('user.account.profile.Verification Code'),
                  })
                "
                autocomplete="off"
              />
                ...
                {{
                  state.dialog.codeSendCountdown <= 0
                    ...
                    : state.dialog.codeSendCountdown +
                      t("user.account.profile.seconds")
                }}
              </el-button>
            </el-col>
          </el-row>
    ...
    <el-dialog
      :title="
        t('user.account.profile.bind') +
        t('user.account.profile.' + state.dialog.type)
      "
    ...
    >
      ...
      >
        ...
        <FormItem
          ...
          :placeholder="
            $t('Please input field', {
              field: t('user.account.profile.New ' + state.dialog.type),
            })
          "
        />
        <FormItem
...
const state: {
  ...
  rules: {
    username: [
      buildValidatorData({
        name: "required",
        title: t("user.account.profile.User name"),
      }),
    ...
    nickname: [
      buildValidatorData({
        name: "required",
        title: t("user.account.profile.nickname"),
      }),
    ],
  },
    ...
    verification: {
      ...
      rules: {
        password: [
          buildValidatorData({
            name: "required",
            title: t("user.account.profile.password"),
          }),
        ...
        captcha: [
          buildValidatorData({
            name: "required",
            title: t("user.account.profile.Verification Code"),
          }),
        ],
      },
      form: {
    ...
    bind: {
      ...
      rules: {
        ...
        email: [
          buildValidatorData({
            name: "required",
            title: t("user.account.profile.email"),
          }),
          buildValidatorData({
            name: "email",
            title: t("user.account.profile.email"),
        ...
        mobile: [
          buildValidatorData({
            name: "required",
            title: t("user.account.profile.mobile"),
          }),
          buildValidatorData({
            name: "mobile",
            title: t("user.account.profile.mobile"),
...
const onChangeBindInfo = (type: "email" | "mobile") => {
  if (
    (type == "email" && userInfo.email) ||
    (type == "mobile" && userInfo.mobile)
  ) {
...
const sendVerificationCaptchaPre = () => {
  ...
  verificationFormRef.value!.validateField("password").then((res) => {
    if (!res) return;
    clickCaptcha(state.dialog.captchaId, (captchaInfo: string) =>
