    ...
    <el-row v-loading="state.loading" :gutter="20">
        ...
        >
          ...
          >
            <el-tab-pane
              ...
              v-for="(group, key) in state.config"
              :key="key"
              :name="key"
              :label="group.title"
            >
                  ...
                  <FormItem
                    ...
                    :input-attr="
                      !isEmpty(item.content)
                        ? { content: item.content, ...item.input_extend }
                        : item.input_extend
                    "
                    :tip="item.tip"
                    :key="'other-' + item.id"
      ...
      <el-col :xs="24" :sm="8">
        <el-card :header="t('routine.config.Quick configuration entry')">
          <el-button
            v-for="(item, idx) in state.quickEntrance"
            class="config_quick_entrance"
            :key="idx"
          >
            <div @click="routePush({ name: item['value'] })">
              {{ item["key"] }}
            </div>
          </el-button>
        </el-card>
...
import {
  ...
  postData,
  postSendTestMail,
} from "/@/api/backend/routine/config";
  ...
  formRef.value?.validate((valid) => {
    if (valid) {
      ...
      for (const key in state.config) {
        ...
        for (const lKey in state.config[key].list) {
          formData[state.config[key].list[lKey].name] =
            state.form[state.config[key].list[lKey].name] ?? "";
        }
      }
      postData("edit", formData).then(() => {
        ...
        if (
          formData.backend_entrance &&
          formData.backend_entrance != adminBaseRoutePath
        ) {
          window.open(
...
const onTestSendMail = () => {
  if (
    !state.form.smtp_server ||
    !state.form.smtp_port ||
    !state.form.smtp_user ||
    !state.form.smtp_pass ||
    !state.form.smtp_sender_mail
  ) {
    ElNotification({
      type: "error",
  ...
  ElMessageBox.prompt(
    t("routine.config.Please enter the recipient email address"),
    t("routine.config.Test mail sending"),
    {
      ...
      inputPattern:
        /[\w!#$%&'*+/=?^_`{|}~-]+(?:\.[\w!#$%&'*+/=?^_`{|}~-]+)*@(?:[\w](?:[\w-]*[\w])?\.)+[\w](?:[\w-]*[\w])?/,
      inputErrorMessage: t(
        "routine.config.Please enter the correct email address",
      ),
