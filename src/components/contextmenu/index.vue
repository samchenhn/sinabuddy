...
const onShowContextmenu = (sourceData: any, axis: Axis) => {
  ...
  const yOffset =
    ...
    axis.y -
    (props.items.length * 40 + 20);
  const xOffset =
    document.documentElement.clientWidth - axis.x - (props.width + 20);
  if (yOffset < 0) {
    axis.y += yOffset;
  ...
  removeEventListenerFn.click = useEventListener(
    document,
    "click",
    onHideContextmenu,
  );
  removeEventListenerFn.scroll = useEventListener(
    document,
    "scroll",
    onHideContextmenu,
  );
