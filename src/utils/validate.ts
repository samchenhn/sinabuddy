...
export function validatorMobile(
  ...
  if (!/^(1[3-9])\d{9}$/.test(mobile.toString())) {
    return callback(
      new Error(
        i18n.global.t("validate.Please enter the correct mobile number"),
      ),
    );
  }
  return callback();
 ...
 * 身份证号验证
 */
export function validatorIdNumber(
  rule: any,
  ...
  if (!/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(idNumber.toString())) {
    return callback(
      new Error(i18n.global.t("validate.Please enter the correct ID number")),
    );
  }
  return callback();
}
...
export function validatorAccount(rule: any, val: string, callback: Function) {
  ...
  if (!/^[a-zA-Z][a-zA-Z0-9_]{2,15}$/.test(val)) {
    return callback(
      new Error(i18n.global.t("validate.Please enter the correct account")),
    );
  }
  return callback();
}
...
export function validatorPassword(rule: any, val: string, callback: Function) {
  ...
  if (!regularPassword(val)) {
    return callback(
      new Error(i18n.global.t("validate.Please enter the correct password")),
    );
  }
  return callback();
}
...
export function validatorVarName(rule: any, val: string, callback: Function) {
  ...
  if (!regularVarName(val)) {
    return callback(
      new Error(i18n.global.t("validate.Please enter the correct name")),
    );
  }
  return callback();
}
...
export function validatorEditorRequired(
  ...
  if (!val || val == "<p><br></p>") {
    return callback(
      new Error(i18n.global.t("validate.Content cannot be empty")),
    );
  }
  return callback();
}
...
export function buildValidatorData({
  ...
  title,
  trigger = "blur",
}: buildValidatorParams): FormItemRule {
  // 必填
  if (name == "required") {
    return {
      required: true,
      message: message
        ? message
        : i18n.global.t("Please input field", { field: title }),
      trigger: trigger,
    };
  }
  ...
  if (validatorType.includes(name)) {
    return {
      ...
      message: message
        ? message
        : i18n.global.t("Please enter the correct field", { field: title }),
      trigger: trigger,
    };
  }
