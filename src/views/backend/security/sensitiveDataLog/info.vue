<template>
  ...
  >
    ...
    >
      ...
      >
        <el-descriptions
          v-if="!isEmpty(baTable.form.extend!.info)"
          :column="2"
          border
        >
          ...
          <el-descriptions-item
            :label="t('security.sensitiveDataLog.Operator IP')"
          >
            {{ baTable.form.extend!.info.ip }}
          </el-descriptions-item>
          <el-descriptions-item
            :label="t('security.sensitiveDataLog.Data table primary key')"
          >
            {{
              baTable.form.extend!.info.primary_key +
              "=" +
              baTable.form.extend!.info.id_value
            }}
          </el-descriptions-item>
          <el-descriptions-item
            :label="t('security.sensitiveDataLog.Modified item')"
          >
            {{
        ...
        <div class="diff-box">
          <div class="diff-box-title">
            {{ t("security.sensitiveDataLog.Modification comparison") }}
          </div>
          <code-diff
            diffStyle="char"
    ...
    <template #footer>
      <el-button
        v-blur
        @click="onRollback(baTable.form.extend!.info.id)"
        type="success"
      >
        <Icon size="16" color="#ffffff" name="fa fa-sign-in" />
        <span class="table-header-operate-text">{{
          t("security.sensitiveDataLog.RollBACK")
        }}</span>
      </el-button>
    </template>
  </el-dialog>
...
const onRollback = (id: string) => {
  ElMessageBox.confirm(
    t(
      "security.sensitiveDataLog.Are you sure you want to rollback the record?",
    ),
    "",
    {
