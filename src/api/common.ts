...
import {
  state as uploadExpandState,
  fileUpload as uploadExpand,
} from "/@/components/mixins/baUpload";
...
export const adminBuildSuffixSvgUrl =
  adminBaseRoutePath + "/ajax/buildSuffixSvg";
...
export const getDatabaseConnectionListUrl =
  "/admin/ajax/getDatabaseConnectionList";
...
export function fileUpload(
  ...
  if (!file.name || typeof file.size == "undefined") {
    errorMsg = i18n.global.t(
      "utils.The data of the uploaded file is incomplete!",
    );
    ...
    errorMsg = i18n.global.t(
      "utils.The size of the uploaded file exceeds the allowed range!",
    );
  }
  if (errorMsg) {
...
export function checkClickCaptcha(
  id: string,
  info: string,
  unset: boolean,
  apiBaseURL: string,
...
export class baTableApi {
  ...
  postData(action: string, data: anyObj) {
    return createAxios(
      {
        url: this.actionUrl.has(action)
          ? this.actionUrl.get(action)
          : this.controllerUrl + action,
        method: "post",
        data,
      },
      {
