  ...
  <div>
    ...
    >
      <TableHeader
        ...
        :quick-search-placeholder="t('crud.log.quick Search Fields')"
      >
        <template v-if="baAccount.token">
          <el-tooltip
            :content="
              t(
                'crud.log.Upload the selected design records to the cloud for cross-device use',
              )
            "
            placement="top"
          >
          ...
          <el-tooltip
            :content="
              t(
                'crud.log.Design records that have been synchronized to the cloud',
              )
            "
            placement="top"
          >
            ...
            >
              ...
              <span class="table-header-operate-text">{{
                t("crud.log.Cloud record")
              }}</span>
            </el-button>
          </el-tooltip>
          <el-button
            v-blur
            @click="toggleShowConfig(true)"
            class="table-header-operate"
            type="primary"
          >
          ...
          <el-button
            v-blur
            @click="toggleShowBaAccount(true)"
            class="table-header-operate"
            type="primary"
          >
            <Icon name="fa fa-user-o" />
            <span class="table-header-operate-text">{{
              t("layouts.Member information")
            }}</span>
          </el-button>
        </template>
        <template v-else>
          ...
          >
            ...
            <span class="table-header-operate-text">{{
              t("crud.log.Login for backup design")
            }}</span>
          </el-button>
        </template>
      </TableHeader>
        ...
        <template #tableName>
          ...
          >
            <template #default="scope">
              {{
                (scope.row.table.databaseConnection
                  ? scope.row.table.databaseConnection + "."
    ...
    <el-dialog v-model="state.showConfig" :title="t('crud.log.Settings')">
      <div
        ...
        :style="config.layout.shrink ? '' : 'width: calc(100% - 90px)'"
      >
        <el-form
          @keyup.enter="onConfigSubmit"
        ...
        >
          <FormItem
            ...
            :input-attr="{
              border: true,
              content: {
                manual: t('crud.log.Manual'),
                automatic: t('crud.log.automatic'),
              },
            }"
            :block-help="
              t(
                'crud.log.You can use the synchronized design records across devices',
              )
            "
          />
          <FormItem
            ...
            :label="
              t(
                'crud.log.When automatically synchronizing records, share them to the open source community',
              )
            "
            ...
            :input-attr="{
              border: true,
              content: {
                no: t('crud.log.Not to share'),
                yes: t('crud.log.Share'),
              },
            }"
            :block-help="
              t(
                'crud.log.Enabling sharing can automatically earn community points during development',
              )
            "
          />
          <FormItem
            :label="
              t(
                'crud.log.The synchronized CRUD records are automatically resynchronized when they are updated',
              )
            "
            ...
            :input-attr="{
              border: true,
              content: {
                no: t('crud.log.Do not resynchronize'),
                yes: t('crud.log.Automatic resynchronization'),
              },
            }"
          />
        </el-form>
      ...
      <template #footer>
        <div :style="'width: calc(100% - 90px)'">
          <el-button @click="toggleShowConfig(false)">{{
            t("Cancel")
          }}</el-button>
          <el-button v-blur @click="onConfigSubmit" type="primary">
            {{ t("Save") }}
          </el-button>
        </div>
      </template>
    ...
    <el-dialog v-model="state.showUpload" :title="t('Upload')" width="60%">
      <div class="ba-operate-form" v-loading="state.uploadValidLoading">
        <el-table
          :empty-text="t('crud.log.No effective design')"
          :data="state.uploadValidData"
          stripe
          class="w100"
        >
          <el-table-column
          ...
          <el-table-column
            prop="fieldCount"
            :label="t('crud.log.Number of fields')"
            align="center"
          />
          ...
          <el-table-column
            prop="score"
            :label="t('crud.log.Share to earn points')"
            align="center"
          >
            <template #default="scope">
              <el-text :type="scope.row.score <= 0 ? 'info' : 'success'">{{
                scope.row.score
              }}</el-text>
            </template>
          </el-table-column>
          <el-table-column
            :label="t('crud.log.Share to the open source community')"
            align="center"
          >
            <template #default="scope">
      ...
      <template #footer>
        <div :style="'width: calc(100% - 90px)'">
          <el-button @click="toggleShowUpload(false)">{{
            t("Cancel")
          }}</el-button>
          <el-button v-blur @click="onSaveUpload" type="primary">
            {{ t("Upload") }}
          </el-button>
        </div>
      </template>
        ...
        >
          <template #reference>
            ...
            >
              ...
              <span class="download-table-header-operate-text">{{
                t("Delete")
              }}</span>
            </el-button>
          </template>
        ...
        <el-table-column :label="t('crud.log.Field')" align="center">
          <template #default="scope">
            <el-popover
              :width="460"
              class="box-item"
              :title="t('crud.log.Field information')"
              placement="left"
            >
              <template #reference>
                <el-text class="cp" type="primary">{{
                  scope.row.fieldCount
                }}</el-text>
              </template>
              <el-table
                :empty-text="t('crud.log.No field')"
              ...
              >
                <el-table-column
                  prop="name"
                  :label="t('crud.log.Field name')"
                  align="center"
                />
                <el-table-column
                  prop="comment"
                  :label="t('crud.log.Note')"
                  align="center"
                  show-overflow-tooltip
                />
                <el-table-column
                  :label="t('crud.log.Type')"
                  align="center"
                  show-overflow-tooltip
                >
        ...
        <el-table-column :label="t('Operate')" align="center">
          <template #default="scope">
            <el-popconfirm
              ...
              @confirm="onLoadLog(scope.row.id)"
            >
              <template #reference>
            ...
            <el-popconfirm
              :title="t('crud.log.Delete cloud records?')"
              @confirm="onDelLog([scope.row.id])"
            >
              <template #reference>
    ...
    <BaAccountDialog
      v-model="state.showBaAccount"
      :login-callback="onBaAccountLoginSuccess"
    />
  </div>
</template>
...
const onSaveUpload = () => {
  ...
  for (const key in selection) {
    ...
    const validDataKey = getArrayKey(
      state.uploadValidData,
      "sync",
      s.id.toString(),
    );
