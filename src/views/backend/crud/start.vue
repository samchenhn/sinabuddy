  ...
  <div class="default-main">
    ...
    <div class="start-opt">
      <el-row :gutter="20">
        <el-col :xs="24" :span="8">
          <div @click="changeStep('create')" class="start-item suspension">
            ...
            <div class="start-item-remark">
              {{ t("crud.crud.New background CRUD from zero") }}
            </div>
          </div>
        </el-col>
        <el-col @click="onShowDialog('db')" :xs="24" :span="8">
          <div class="start-item suspension">
            ...
            <div class="start-item-remark">
              {{
                t("crud.crud.Select a designed data table from the database")
              }}
            </div>
          </div>
        </el-col>
        <el-col @click="state.showLog = true" :xs="24" :span="8">
          <div class="start-item suspension">
            ...
            <div class="start-item-remark">
              {{ t("crud.crud.Start with previously generated CRUD code") }}
            </div>
          </div>
        </el-col>
      </el-row>
        ...
        <el-col :span="20" class="ba-markdown crud-tips suspension">
          ...
          <ol>
            <li>
              ...
              <a
                target="_blank"
                href="https://doc.buildadmin.com/guide/other/developerMustSee.html"
            ...
            <li>
              ...
              {{ t("crud.crud.experience 2 3") }} <code>test_build</code>
              {{ t("crud.crud.data sheet") }}
            </li>
            <li>
              {{ t("crud.crud.experience 3 1") }}
              <code>{{ t("crud.crud.experience 3 2") }}</code>
          ...
          >
            <template #title>
              ...
              <span>
                {{ t("crud.crud.experience 4 3")
                }}<code>{{ t("crud.crud.experience 4 4") }}</code>
              </span>
            </template>
          </el-alert>
        </el-col>
      ...
      <el-dialog
        ...
        :title="
          state.dialog.type == 'sql'
            ? t('crud.crud.Please enter SQL')
            : t('crud.crud.Please select a data table')
        "
      ...
      >
        ...
        >
          ...
          <template v-else-if="state.dialog.type == 'db'">
            <FormItem
              ...
              :placeholder="
                t('Please select field', { field: t('Database connection') })
              "
            />
            <FormItem
            ...
            <el-alert
              ...
              :title="
                t(
                  'crud.crud.The selected table has already generated records You are advised to start with historical records',
                )
              "
              :show-icon="true"
              :closable="false"
        ...
        <template #footer>
          <div :style="{ width: 'calc(100% * 0.9)' }">
            <el-button @click="state.dialog.visible = false">{{
              $t("Cancel")
            }}</el-button>
            <el-button
...
const rules: Partial<Record<string, FormItemRule[]>> = reactive({
  table: [
    buildValidatorData({
      name: "required",
      message: t("crud.crud.Please select a data table"),
    }),
  ],
});
...
const onTableStartChange = () => {
  if (crudState.startData.table) {
    ...
    checkCrudLog(
      crudState.startData.table,
      crudState.startData.databaseConnection,
    )
      .then((res) => {
