  ...
  <div>
    <template v-if="isArray(cellValue)">
      <template v-for="(tag, idx) in cellValue" :key="idx">
        <el-tag
          ...
          v-bind="
            invokeTableContextDataFun(field.customRenderAttr?.tag, {
              row,
              field,
              cellValue,
              column,
        ...
        >
          {{
            !isEmpty(field.replaceValue)
              ? (field.replaceValue[tag] ?? tag)
              : tag
          }}
        </el-tag>
      </template>
    ...
    <template v-else>
      ...
      >
        {{
          !isEmpty(field.replaceValue)
            ? (field.replaceValue[cellValue] ?? cellValue)
            : cellValue
        }}
      </el-tag>
    </template>
