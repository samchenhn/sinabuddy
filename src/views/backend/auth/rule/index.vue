  ...
  <div class="default-main ba-table-box">
    ...
    <TableHeader
      ...
      :quick-search-placeholder="
        t('Quick search placeholder', { fields: t('auth.rule.title') })
      "
    />
...
const baTable = new baTableClass(
  new baTableApi("/admin/auth.Rule/"),
  {
    ...
    column: [
      ...
      {
        label: t("auth.rule.title"),
        prop: "title",
        align: "left",
        width: "200",
      },
      {
        label: t("auth.rule.Icon"),
        prop: "icon",
        align: "center",
        width: "60",
        render: "icon",
...
const onExpandChange = (row: any, expanded: boolean) => {
  if (expanded) {
    ...
    sessionState.expanded = sessionState.expanded.filter(
...
const restoreState = () => {
  nextTick(() => {
    ...
    for (const key in sessionStateTemp.expanded) {
      tableRef.value
        ?.getRef()
        ?.toggleRowExpansion(sessionStateTemp.expanded[key], true);
    }
    nextTick(() => {
      if (sessionStateTemp.scrollTop || sessionStateTemp.scrollLeft) {
        tableRef.value?.getRef()?.scrollTo({
          top: sessionStateTemp.scrollTop || 0,
          left: sessionStateTemp.scrollLeft || 0,
        });
      }
