...
export default class baTable {
  ...
  onSubmit = (formEl?: FormInstance | null) => {
    // 当前操作的首字母小写
    const operate = this.form.operate!.replace(
      this.form.operate![0],
      this.form.operate![0].toLowerCase(),
    );
  ...
  initSort = () => {
    if (this.table.defaultOrder && this.table.defaultOrder.prop) {
      if (!this.table.ref) {
        console.warn(
          "Failed to initialize default sorting because table ref is not defined. Please assign table ref when onMounted",
        );
        return;
      }

      const defaultOrder =
        this.table.defaultOrder.prop + "," + this.table.defaultOrder.order;
      if (this.table.filter && this.table.filter.order != defaultOrder) {
        this.table.filter.order = defaultOrder;
        this.table.ref
          .getRef()
          ?.sort(
            this.table.defaultOrder.prop,
            this.table.defaultOrder.order == "desc"
              ? "descending"
              : "ascending",
          );
      }
  ...
  dragSort = () => {
    ...
    const el = this.table.ref
      .getRef()
      ?.$el.querySelector(".el-table__body-wrapper .el-table__body tbody");
    const disabledTip =
      this.table.column[buttonsKey].buttons![moveButton].disabledTip;
    Sortable.create(el, {
      animation: 200,
      ...
      onEnd: (evt: Sortable.SortableEvent) => {
        ...
        if (
          ...
          moveRow[this.table.dragSortLimitField] !=
            targetRow[this.table.dragSortLimitField]
        ) {
          this.onTableHeaderAction("refresh", {});
          ElNotification({
            type: "error",
            message: i18n.global.t(
              "utils.The moving position is beyond the movable range!",
            ),
          });
          return;
        }
  ...
  mount = () => {
    ...
    if (this.table.acceptQuery && !isEmpty(route.query)) {
      ...
      // 获取公共搜索数据合并至表格筛选条件
      this.table.filter!.search = this.getComSearchData().concat(
  ...
  setComSearchData = (query: anyObj) => {
    for (const key in this.table.column) {
      ...
      if (prop && typeof query[prop] !== "undefined") {
        ...
        if (
          this.table.column[key].operator == "RANGE" ||
          this.table.column[key].operator == "NOT RANGE"
        ) {
          const range = queryProp.split(",");
          if (
            this.table.column[key].render == "datetime" ||
            this.table.column[key].comSearchRender == "date"
          ) {
              ...
              if (rangeDayJs[0].isValid() && rangeDayJs[1].isValid()) {
                if (this.table.column[key].comSearchRender == "date") {
                  this.comSearch.form[prop] = [
                    rangeDayJs[0].format("YYYY-MM-DD"),
                    rangeDayJs[1].format("YYYY-MM-DD"),
                  ];
                } else {
                  this.comSearch.form[prop] = [
          ...
          this.table.column[key].operator == "NULL" ||
          this.table.column[key].operator == "NOT NULL"
        ) {
  ...
  getComSearchData = () => {
    ...
    for (const key in this.comSearch.form) {
      ...
      if (
        ...
        (fieldDataTemp.operator == "RANGE" ||
          fieldDataTemp.operator == "NOT RANGE")
      ) {
        // 时间范围
        if (this.comSearch.form[key] && this.comSearch.form[key].length >= 2) {
          if (fieldDataTemp.comSearchRender == "date") {
            val =
              this.comSearch.form[key][0] +
              " 00:00:00" +
              "," +
              this.comSearch.form[key][1] +
              " 23:59:59";
          } else {
            val =
              this.comSearch.form[key][0] + "," + this.comSearch.form[key][1];
          }
        }
      } else if (
        fieldDataTemp.operator == "RANGE" ||
        ...
        if (
          !this.comSearch.form[key + "-start"] &&
          !this.comSearch.form[key + "-end"]
        ) {
          continue;
        }
        val =
