  ...
  () => {
    ...
    function _addTab(route: RouteLocationNormalized) {
      ...
      for (const key in state.tabsView) {
        ...
        if (
          state.tabsView[key].meta.id === tabView.meta.id ||
          state.tabsView[key].fullPath == tabView.fullPath
        ) {
          state.tabsView[key].fullPath = tabView.fullPath;
          state.tabsView[key].params = !isEmpty(tabView.params)
            ? tabView.params
            : state.tabsView[key].params;
          state.tabsView[key].query = !isEmpty(tabView.query)
            ? tabView.query
      ...
      if (typeof tabView.meta.title == "string") {
        tabView.meta.title = i18n.global.te(tabView.meta.title)
          ? i18n.global.t(tabView.meta.title)
          : tabView.meta.title;
      }
     ...
     */
    const _closeTabs = (
      retainMenu: RouteLocationNormalized | false = false,
    ) => {
      if (retainMenu) {
    ...
    const getTabsViewDataByRoute = (
      ...
      // 以完整路径寻找
      let found = getTabsViewDataByPath(
        route.fullPath,
     ...
     * @param returnType 返回值要求:normal=返回被搜索的路径对应的菜单数据,above=返回被搜索的路径对应的上一级菜单数组
     */
    const getTabsViewDataByPath = (
      path: string,
      menus: RouteRecordRaw[],
      returnType: "normal" | "above",
    ): RouteRecordRaw | false => {
      for (const key in menus) {
        ...
        if (menus[key].children && menus[key].children.length) {
          const find = getTabsViewDataByPath(
            path,
            menus[key].children,
            returnType,
          );
...
function encodeRoutesURI(data: RouteRecordRaw[]) {
  data.forEach((item) => {
    if (item.meta?.menu_type == "iframe") {
      item.path =
        adminBaseRoutePath + "/iframe/" + encodeURIComponent(item.path);
    }
