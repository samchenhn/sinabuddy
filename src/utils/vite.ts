...
export function init() {
  if (import.meta.hot) {
    ...
    import.meta.hot.on(
      "custom:change-hot-update-state",
      (state: Partial<HotUpdateState>) => {
        ...
        hotUpdateState.listenDirtyFileSwitch =
          state.listenDirtyFileSwitch ?? hotUpdateState.listenDirtyFileSwitch;
      },
    );
...
export const customHotUpdate = (): Plugin => {
  ...
  return {
    ...
    configureServer(server) {
      ...
      // 修改监听脏文件的开关
      server.ws.on(
        "custom:change-listen-dirty-file-switch",
        (status: boolean) => {
