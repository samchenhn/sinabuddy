...
function createAxios<Data = any, T = ApiPromise<Data>>(
  ...
  // 自定义后台入口
  if (
    adminBaseRoute.path != "/admin" &&
    isAdminApp() &&
    /^\/admin\//.test(axiosConfig.url!)
  ) {
    axiosConfig.url = axiosConfig.url!.replace(
      /^\/admin\//,
      adminBaseRoute.path + ".php/",
    );
  }
      ...
      if (response.config.responseType == "json") {
        if (response.data && response.data.code !== 1) {
          ...
          if (response.data.code == 303) {
            ...
            let routerPath = isAdminAppFlag
              ? adminBaseRoute.path
              : memberCenterBaseRoutePath;
          ...
          ElNotification({
            message: response.data.msg
              ? response.data.msg
              : i18n.global.t("axios.Operation successful"),
            type: "success",
            zIndex: SYSTEM_ZINDEX,
          });
        }
