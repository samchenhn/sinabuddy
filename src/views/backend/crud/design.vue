<template>
  <div class="default-main">
    <div class="header-config-box">
        ...
        <div class="header">
          <div class="header-item-box">
            ...
            <FormItem
              ...
              :placeholder="
                t(
                  'crud.crud.For example: `user table` will be generated into `user management`',
                )
              "
            />
          </div>
      ...
      <transition
        :name="
          state.showHeaderSeniorConfig ? 'el-zoom-in-top' : 'el-zoom-in-bottom'
        "
      >
        <div
          v-if="state.showHeaderSeniorConfig"
          class="header-senior-config-box"
        >
          <div class="header-senior-config-form">
            <el-form-item
              :label-width="140"
              :label="t('crud.crud.Table Quick Search Fields')"
            >
              <el-select
                :clearable="true"
                :multiple="true"
                class="w100"
                v-model="state.table.quickSearchField"
                placement="bottom"
              >
                <el-option
                  ...
                  :key="idx"
                  :label="
                    item.name + (item.comment ? '-' + item.comment : item.title)
                  "
                  :value="item.name"
                />
              </el-select>
            ...
            <div class="default-sort-field-box">
              ...
              >
                <el-select
                  :clearable="true"
                  v-model="state.table.defaultSortField"
                  placement="bottom"
                >
                  <el-option
                    v-for="(item, idx) in state.fields"
              ...
              <FormItem
                ...
                :input-attr="{
                  content: {
                    desc: t('crud.crud.sort order desc'),
                    asc: t('crud.crud.sort order asc'),
                  },
                }"
              />
            </div>
            <el-form-item
              :label-width="140"
              :label="t('crud.crud.Fields as Table Columns')"
            >
              <el-select
                :clearable="true"
                :multiple="true"
                class="w100"
                v-model="state.table.columnFields"
                placement="bottom"
              >
                <el-option
                  v-for="(item, idx) in state.fields"
            ...
            <el-form-item
              :label-width="140"
              :label="t('crud.crud.Fields as form items')"
            >
              <el-select
                :clearable="true"
                :multiple="true"
                class="w100"
                v-model="state.table.formFields"
                placement="bottom"
              >
                <el-option
                  v-for="(item, idx) in state.fields"
            ...
            <FormItem
              ...
              :block-help="
                t(
                  'crud.crud.For quick combination code generation location, please fill in the relative path',
                )
              "
              :input-attr="{
      ...
      <div
        @click="state.showHeaderSeniorConfig = !state.showHeaderSeniorConfig"
        class="header-senior-config"
      >
        ...
        <el-collapse class="field-collapse" v-model="state.fieldCollapseName">
          <el-collapse-item :title="t('crud.crud.Common Fields')" name="common">
            <div class="field-box" :ref="tabsRefs.set">
              <div
                v-for="(field, index) in fieldItem.common"
                :key="index"
                class="field-item"
              >
          ...
          <el-collapse-item :title="t('crud.crud.Base Fields')" name="base">
            <div class="field-box" :ref="tabsRefs.set">
              <div
                v-for="(field, index) in fieldItem.base"
                :key="index"
                class="field-item"
              >
                <span>{{ field.title }}</span>
          ...
          <el-collapse-item
            :title="t('crud.crud.Advanced Fields')"
            name="senior"
          >
            <div class="field-box" :ref="tabsRefs.set">
              <div
                v-for="(field, index) in fieldItem.senior"
                :key="index"
                class="field-item"
              >
                <span>{{ field.title }}</span>
        ...
        <div ref="designWindowRef" class="design-window ba-scroll-style">
          ...
          >
            ...
            <div class="design-field-right">
              <el-button
                v-if="
                  ['remoteSelect', 'remoteSelects'].includes(field.designType)
                "
          ...
          <div
            class="design-field-empty"
            v-if="!state.fields.length && !state.draggingField"
          >
            {{
          ...
          <div v-else :key="'activate-field-' + state.activateField">
            <el-form label-position="top">
              ...
              <FormItem
                ...
                :input-attr="{
                  onInput: ($event: string) =>
                    onFieldNameChange($event, state.activateField),
                }"
              />
              ...
              <el-form-item :label="t('crud.crud.Field Defaults')">
                <el-select
                  v-model="state.fields[state.activateField].defaultType"
                >
                  <el-option label="手动输入" value="INPUT" />
                  <el-option
                    label="EMPTY STRING（空字符串）"
                    value="EMPTY STRING"
                  />
                ...
                <el-input
                  v-if="
                    state.fields[state.activateField].defaultType == 'INPUT'
                  "
              ...
              <template
                v-if="!isEmpty(state.fields[state.activateField].table)"
              >
                <el-divider content-position="left">{{
                  t("crud.crud.Field Table Properties")
                }}</el-divider>
                <template
                  v-for="(item, idx) in state.fields[state.activateField].table"
                  :key="idx"
                >
                  <FormItem
                    ...
                    :placeholder="
                      state.fields[state.activateField].table[idx]
                        .placeholder ?? ''
                    "
                    :input-attr="{
                      content:
                        state.fields[state.activateField].table[idx].options ??
                        {},
              ...
              <template v-if="!isEmpty(state.fields[state.activateField].form)">
                ...
                <template
                  v-for="(item, idx) in state.fields[state.activateField].form"
                  :key="idx"
                >
                  <FormItem
                    ...
                    :placeholder="
                      state.fields[state.activateField].form[idx].placeholder ??
                      ''
                    "
                    :input-attr="{
                      content:
                        state.fields[state.activateField].form[idx].options ??
                        {},
    ...
    >
        ...
        <div class="ba-operate-form" :style="'width: calc(100% - 80px)'">
          <el-form
            ...
            v-if="
              ...
              state.fields[state.remoteSelectPre.index]
            "
          >
            <FormItem
            ...
            <div v-loading="state.loading.remoteSelect">
              <FormItem
                ...
                :placeholder="
                  t(
                    'crud.crud.Please select the value field of the select component',
                  )
                "
                :key="
                  'select-value' +
                  JSON.stringify(state.remoteSelectPre.fieldList)
                "
                :input-attr="{
              ...
              <FormItem
                ...
                :placeholder="
                  t(
                    'crud.crud.Please select the label field of the select component',
                  )
                "
                :key="
                  'select-label' +
              ...
              <FormItem
                v-if="
                  state.fields[state.remoteSelectPre.index].designType ==
                  'remoteSelect'
                "
                prop="joinField"
                type="selects"
                ...
                :placeholder="
                  t('crud.crud.Please select the fields displayed in the table')
                "
                :key="
                  'join-field' + JSON.stringify(state.remoteSelectPre.fieldList)
                "
                :input-attr="{
                  content: state.remoteSelectPre.fieldList,
                }"
              ...
              <FormItem
                ...
                :input-attr="{
                  ...
                  content: {
                    crud: t(
                      'crud.crud.Fast configuration with generated controllers and models',
                    ),
              ...
              <FormItem
                ...
                :placeholder="
                  t('crud.crud.Please select the controller of the data table')
                "
                :key="
                  'controller-file' +
                  JSON.stringify(state.remoteSelectPre.controllerFileList)
                "
                :input-attr="{
              ...
              <FormItem
                ...
                :placeholder="
                  t(
                    'crud.crud.Please select the data model location of the data table',
                  )
                "
                :key="
                  'model-file' +
                  JSON.stringify(state.remoteSelectPre.modelFileList)
                "
                :input-attr="{
              ...
              <el-form-item
                v-if="
                  state.table.databaseConnection &&
                  state.remoteSelectPre.form.modelFile
                "
                :label="t('Database connection')"
              >
                ...
                <div class="block-help">
                  ...
                  <div>
                    {{
                      t(
                        "crud.crud.There is no connection attribute in model class",
                      )
                    }}
                  </div>
                </div>
              ...
              >
                <template #append
                  >.{{ state.remoteSelectPre.form.pk }}</template
                >
              </FormItem>
      ...
      <template #footer>
        <div :style="'width: calc(100% - 88px)'">
          <el-button @click="onCancelRemoteSelect">{{
            $t("Cancel")
          }}</el-button>
    ...
    >
      <div class="confirm-generate-dialog-body">
        <el-alert
          ...
          :title="
            t(
              'crud.crud.The controller already exists Continuing to generate will automatically overwrite the existing code!',
            )
          "
          center
          type="error"
      ...
      <template #footer>
        <div class="confirm-generate-dialog-footer">
          <el-button @click="closeConfirmGenerate">{{
            $t("Cancel")
          }}</el-button>
          <el-button
            :loading="state.loading.generate"
    ...
    <el-dialog
      class="ba-operate-dialog design-change-log-dialog"
      width="20%"
      v-model="state.showDesignChangeLog"
    >
      <template #header>
          ...
          <el-timeline class="design-change-log-timeline">
            ...
            >
              <div class="design-timeline-box">
                <el-checkbox
                  v-model="item.sync"
                  :label="getTableDesignChangeContent(item)"
                  size="small"
                />
              </div>
            </el-timeline-item>
          </el-timeline>
          <span class="design-change-tips">{{
...
import {
  ...
  uploadCompleted,
  uploadLog,
} from "/@/api/backend/crud";
import {
  getDatabaseConnectionListUrl,
  getTableFieldList,
  getTableListUrl,
} from "/@/api/common";
...
const onFieldDesignTypeChange = (designType: string) => {
  ...
  for (const key in fieldItem) {
    const fieldItemIndex = getArrayKey(
      fieldItem[key as keyof typeof fieldItem],
      "designType",
      designType,
    );
    if (fieldItemIndex !== false) {
      fieldDesignData = cloneDeep(
        fieldItem[key as keyof typeof fieldItem][fieldItemIndex],
      );
      break;
    }
  }
  ...
  ElMessageBox.confirm(t("crud.crud.Reset generate type attr"), t("Reminder"), {
    ...
    .then(() => {
      ...
      // 远程下拉参数预填
      if (
        ["remoteSelect", "remoteSelects"].includes(fieldDesignData.designType)
      ) {
...
const onFieldNameChange = (val: string, index: number) => {
  ...
  if (nameRepeatKey !== false) {
    ...
    state.error.fieldNameDuplication = ElMessage({
      message:
        t("crud.crud.Rename failed") +
        "：" +
        t("crud.crud.Field name duplication", { field: val }),
      type: "error",
    });
    return;
  }
  ...
  for (const key in tableFieldsKey) {
    for (const idx in state.table[
      tableFieldsKey[key] as TableKey
    ] as string[]) {
      if (
        (state.table[tableFieldsKey[key] as TableKey] as string[])[idx] ==
        oldName
      ) {
 ...
 * 全部字段的名称重复检测
 */
const fieldNameDuplicationCheck = (
  showErrorType: "ElNotification" | "ElMessage",
) => {
  ...
  for (const key in state.fields) {
    ...
    for (const checkKey in state.fields) {
      ...
      if (count > 1) {
        let msg = t("crud.crud.Field name duplication", {
          field: state.fields[key].name,
        });
        if (showErrorType == "ElMessage") {
          state.error.fieldNameDuplication = ElMessage({
...
const clearFieldTableData = (name: string) => {
  ...
  for (const key in tableFieldsKey) {
    const delIdx = (
      state.table[tableFieldsKey[key] as TableKey] as string[]
    ).findIndex((item) => {
      return item == name;
    });
    if (delIdx != -1) {
      (state.table[tableFieldsKey[key] as TableKey] as string[]).splice(
        delIdx,
        1,
      );
    }
...
const showRemoteSelectPre = (index: number, hideDelField = false) => {
  ...
  if (state.fields[index] && state.fields[index].form["remote-table"].value) {
    state.remoteSelectPre.form.table =
      state.fields[index].form["remote-table"].value;
    state.remoteSelectPre.form.pk = state.fields[index].form["remote-pk"].value;
    state.remoteSelectPre.form.label =
      state.fields[index].form["remote-field"].value;
    state.remoteSelectPre.form.controllerFile =
      state.fields[index].form["remote-controller"].value;
    state.remoteSelectPre.form.modelFile =
      state.fields[index].form["remote-model"].value;
    state.remoteSelectPre.form.remoteUrl =
      state.fields[index].form["remote-url"].value;
    state.remoteSelectPre.form.sourceConfigType =
      state.fields[index].form["remote-source-config-type"].value;
    state.remoteSelectPre.form.primaryTableAlias =
      state.fields[index].form["remote-primary-table-alias"].value;
    state.remoteSelectPre.form.joinField =
      state.fields[index].form["relation-fields"].value.split(",");
    getTableFieldList(
      state.fields[index].form["remote-table"].value,
      true,
      state.table.databaseConnection,
    ...
    if (
      isEmpty(state.remoteSelectPre.modelFileList) ||
      isEmpty(state.remoteSelectPre.controllerFileList)
    ) {
      getFileData(state.fields[index].form["remote-table"].value).then(
        (res) => {
          state.remoteSelectPre.modelFileList = res.data.modelFileList;
          state.remoteSelectPre.controllerFileList =
            res.data.controllerFileList;
        },
      );
    }
  }
...
const onEditField = (index: number, field: FieldItem) => {
  if (["remoteSelect", "remoteSelects"].includes(field.designType))
    return showRemoteSelectPre(index);
};
...
const startGenerate = () => {
  ...
  generate({
    ...
    .then((res) => {
      ...
      if (
        (state.sync > 0 && config.crud.syncedUpdate === "yes") ||
        (state.sync == 0 && config.crud.syncType == "automatic")
      ) {
        uploadLog({
          logs: [
            {
...
const onGenerate = () => {
  ...
  if (state.error.tableName)
    msg = t("crud.crud.Please enter the correct table name!");
  ...
  generateCheck({
    ...
    .catch((res) => {
      ...
      if (res.code == -1) {
        ...
        if (
          showTableConflictConfirmGenerate() ||
          state.confirmGenerate.controller ||
          state.confirmGenerate.menu
        ) {
...
const showTableConflictConfirmGenerate = () =>
  state.confirmGenerate.table &&
  (crudState.type == "create" || state.table.rebuild == "Yes");
...
const onAbandonDesign = () => {
  ...
  ElMessageBox.confirm(
    t(
      "crud.crud.It is irreversible to give up the design Are you sure you want to give up?",
    ),
    t("Reminder"),
    {
...
const handleFieldAttr = (field: FieldItem) => {
  ...
  for (const tKey in field.form) {
    if (designTypeAttr.form[tKey])
      designTypeAttr.form[tKey].value = field.form[tKey];
    if (tKey == "image-multi" && field.form[tKey]) {
      designTypeAttr.table["render"] = getTableAttr("render", "images");
    }
  }
  for (const tKey in field.table) {
    if (designTypeAttr.table[tKey])
      designTypeAttr.table[tKey].value = field.table[tKey];
  }
...
const onFieldCommentChange = (comment: string) => {
  ...
  if (["enum", "set"].includes(state.fields[state.activateField].type)) {
    if (!comment) {
      state.fields[state.activateField].dataType =
        `${state.fields[state.activateField].type}()`;
      return;
    }
    ...
    if (comments[1]) {
      ...
      // 字段数据类型
      state.fields[state.activateField].dataType =
        `${state.fields[state.activateField].type}(${comments.join(",")})`;
    }
  }
};
  ...
  if (crudState.type == "log") {
    postLogStart(crudState.startData.logId, crudState.startData.logType)
      .then((res) => {
        ...
        state.table.databaseConnection = res.data.table.databaseConnection
          ? res.data.table.databaseConnection
          : "";
        ...
        for (const key in fields) {
          ...
          if (typeof field.defaultType == "undefined") {
            if (
              field.default &&
              ["none", "null", "empty string"].includes(field.default)
            ) {
              field.defaultType = field.default.toUpperCase() as
                | "EMPTY STRING"
                | "NULL"
                | "NONE";
              field.default = "";
...
onMounted(() => {
  ...
  const sortable = Sortable.create(designWindowRef.value!, {
    ...
    onAdd: (evt: SortableEvt) => {
      ...
      if (field && field[evt.oldIndex!]) {
        ...
        logTableDesignChange({
          ...
          after:
            evt.newIndex === 0
              ? "FIRST FIELD"
              : state.fields[evt.newIndex! - 1].name,
        });
      ...
      nextTick(() => {
        sortable.sort(
          range(state.fields.length).map((value) => value.toString()),
        );
      });
    },
    onEnd: (evt) => {
...
const onTableNameChange = (val: string) => {
  ...
  if (/^[a-z_][a-z0-9_]*$/.test(val)) {
    ...
    state.error.tableName = t(
      "crud.crud.Use lower case underlined for table names",
    );
  }
  tableDesignChangeInit();
};
...
const onJoinTableChange = () => {
  ...
  getFileData(state.remoteSelectPre.form.table).then((res) => {
    ...
    if (
      Object.keys(res.data.controllerFileList).includes(res.data.controllerFile)
    ) {
...
const onSaveRemoteSelect = () => {
  const submitCallback = () => {
    ...
    state.fields[state.remoteSelectPre.index].form["remote-table"].value =
      state.remoteSelectPre.form.table;
    state.fields[state.remoteSelectPre.index].form["remote-pk"].value =
      state.remoteSelectPre.form.pk;
    state.fields[state.remoteSelectPre.index].form["remote-field"].value =
      state.remoteSelectPre.form.label;
    state.fields[state.remoteSelectPre.index].form["remote-controller"].value =
      state.remoteSelectPre.form.controllerFile;
    state.fields[state.remoteSelectPre.index].form["remote-model"].value =
      state.remoteSelectPre.form.modelFile;
    state.fields[state.remoteSelectPre.index].form["remote-url"].value =
      state.remoteSelectPre.form.remoteUrl;
    state.fields[state.remoteSelectPre.index].form[
      "remote-source-config-type"
    ].value = state.remoteSelectPre.form.sourceConfigType;
    state.fields[state.remoteSelectPre.index].form[
      "remote-primary-table-alias"
    ].value = state.remoteSelectPre.form.primaryTableAlias;
...
const onCancelRemoteSelect = () => {
  ...
  if (
    state.remoteSelectPre.index !== -1 &&
    state.remoteSelectPre.hideDelField
  ) {
...
const resetRemoteSelectForm = (excludes: string[] = []) => {
  for (const key in state.remoteSelectPre.form) {
    ...
    if (key == "joinField") {
      ...
      (state.remoteSelectPre.form[
        key as keyof typeof state.remoteSelectPre.form
      ] as string) = "";
    }
  }
};
...
const remoteSelectPreFormRules: Partial<Record<string, FormItemRule[]>> =
  reactive({
    table: [
      buildValidatorData({
        name: "required",
        title: t("crud.crud.remote-table"),
      }),
    ],
    pk: [
      buildValidatorData({
        name: "required",
        title: t("crud.crud.Drop down value field"),
      }),
    ],
    label: [
      buildValidatorData({
        name: "required",
        title: t("crud.crud.Drop down label field"),
      }),
    ],
    joinField: [
      buildValidatorData({
        name: "required",
        title: t("crud.crud.Fields displayed in the table"),
      }),
    ],
    controllerFile: [
      buildValidatorData({
        name: "required",
        title: t("crud.crud.Controller position"),
      }),
    ],
    modelFile: [
      buildValidatorData({
        name: "required",
        title: t("crud.crud.Data Model Location"),
      }),
    ],
    remoteUrl: [
      buildValidatorData({
        name: "required",
        title: t("crud.crud.remote-url"),
      }),
    ],
  });
...
const logTableDesignChange = (data: TableDesignChange) => {
  ...
  if (data.type == "change-field-name") {
    for (const key in state.table.designChange) {
      ...
      if (
        state.table.designChange[key].type == "change-field-attr" &&
      ...
      // 有排序记录的字段被改名
      if (
        state.table.designChange[key].type == "change-field-order" &&
        data.oldName == state.table.designChange[key].oldName
      ) {
        state.table.designChange[key].oldName = data.newName;
      }
    ...
    for (const key in state.table.designChange) {
      // 新增字段改名
      if (
        state.table.designChange[key].type == "add-field" &&
        state.table.designChange[key].newName == data.oldName
      ) {
      ...
      // 字段再次改名
      if (
        state.table.designChange[key].type == "change-field-name" &&
        state.table.designChange[key].newName == data.oldName
      ) {
        data.oldName = state.table.designChange[key].oldName;
        ...
        // 取消字段改名
        if (
          state.table.designChange[key].newName ==
          state.table.designChange[key].oldName
        ) {
    ...
    state.table.designChange = state.table.designChange.filter((item) => {
      ...
      // 有改名记录的字段被删除
      const name =
        item.type == "change-field-name" && item.newName == data.oldName;
      // 有属性修改记录的字段被删除
      const attr =
        item.type == "change-field-attr" && item.oldName == data.oldName;
      // 有排序记录的字段被删除
      const order =
        item.type == "change-field-order" && item.oldName == data.oldName;
    ...
    for (const key in state.table.designChange) {
      // 同一字段名称多次删除（删除后添加再删除）
      if (
        state.table.designChange[key].type == "del-field" &&
        state.table.designChange[key].oldName == data.oldName
      ) {
        push = false;
        break;
      }
  ...
  switch (data.type) {
    ...
    case "change-field-order":
      return (
        ...
        data.oldName +
        " => " +
        (data.after == "FIRST FIELD"
          ? t("crud.crud.First field")
          : data.after + " " + t("crud.crud.After"))
      );
    default:
      return t("Unknown");
  }
...
.design-window {
  ...
  background-color: var(--ba-bg-color-overlay);
  border: v-bind(
    'state.draggingField ? "1px dashed var(--el-color-primary)":(state.fields.length ? "none":"1px dashed var(--el-border-color)")'
  );
  .design-field-box {
    display: flex;
    padding: 10px;
