...
const getModules = () => {
  if (
    typeof state.table.modulesEbak[state.table.params.activeTab] != "undefined"
  ) {
    state.table.modules[state.table.params.activeTab] = modulesOnlyLocalHandle(
      state.table.modulesEbak[state.table.params.activeTab],
    );
  ...
  modules(params)
    .then((res) => {
        ...
        state.installedModule.forEach((item) => {
          if (moduleUids.indexOf(item.uid) === -1) {
            if (state.table.params.quickSearch) {
              if (item.title.includes(state.table.params.quickSearch))
      ...
      state.table.modulesEbak[params.activeTab] = res.data.rows.map(
        (item: anyObj) => {
      ...
      state.table.modules[params.activeTab] = modulesOnlyLocalHandle(
        state.table.modulesEbak[params.activeTab],
      );
    ...
    .then((res) => {
      if (localItem) {
        if (res.data.info.type == "local") {
          ...
          res.data.info.images = [
            fullUrl("/static/images/local-module-logo.png"),
          ];
        ...
        res.data.info.enable =
          localItem.state === moduleInstallState.DISABLE ? false : true;
      } else {
...
export const showCommonLoading = (
  loadingTitle: moduleState["common"]["loadingTitle"],
) => {
    ...
    if (
      ...
      ElNotification({
        ...
        message:
          ...
          res.data.state === moduleInstallState.DISABLE
            ? i18n.global.t(
                "module.Installation cancelled because module already exists!",
              )
            : i18n.global.t(
                "module.Installation cancelled because the directory required by the module is occupied!",
              ),
      });
      state.dialog.common = false;
    } else {
      showCommonLoading(
        res.data.state === moduleInstallState.UNINSTALLED
          ? "download"
          : "install",
      );
      execInstall(uid, id);
  ...
  postInstallModule(uid, id, extend)
    ...
    .catch((res) => {
      ...
      if (res.code == -1) {
        ...
        state.common.dialogTitle = i18n.global.t(
          "module.A conflict is found Please handle it manually",
        );
        ...
        state.common.uid = res.data.uid;
        state.common.dialogTitle = i18n.global.t(
          "module.Wait for dependent installation",
        );
...
const terminalTaskExecComplete = (res: number, type: string) => {
  if (res == taskStatus.Success) {
    state.common.waitInstallDepend = state.common.waitInstallDepend.filter(
      (depend: string) => {
        return depend != type;
      },
    );
...
export const onDisable = (confirmConflict = false) => {
  ...
  if (confirmConflict) {
    ...
    for (const key in state.common.disableDependConflict) {
      ...
      if (
        typeof dependConflict[state.common.disableDependConflict[key].env] ==
        "undefined"
      ) {
        dependConflict[state.common.disableDependConflict[key].env] = [];
      }
      dependConflict[state.common.disableDependConflict[key].env].push(
        state.common.disableDependConflict[key].depend,
      );
    }
    ...
    .then(() => {
      ElNotification({
        ...
        message: i18n.global.t(
          "module.The operation succeeds Please clear the system cache and refresh the browser ~",
        ),
        zIndex: SYSTEM_ZINDEX,
      });
      state.dialog.common = false;
...
export const specificUserName = (userInfo: Partial<UserInfo>) => {
  return (
    userInfo.nickname +
    "（" +
    (userInfo.email || userInfo.mobile || "ID:" + userInfo.id) +
    "）"
  );
};
