        ...
        >
          ...
          >
            <el-tooltip
              placement="right"
              effect="light"
              v-if="!isEmpty(tooltipParams)"
            >
              <template #content>
                <p v-for="(tooltipParam, key) in tooltipParams" :key="key">
...
type ElSelectProps = Omit<
  Partial<InstanceType<typeof ElSelect>["$props"]>,
  "modelValue"
>;
...
const emptyValues = computed(
  () =>
    (attrs.emptyValues as InputAttr["emptyValues"]) || [null, undefined, ""],
);
...
const getData = debounce((initValue: valueTypes = "") => {
  ...
  getSelectData(props.remoteUrl, state.keyword, state.params)
    .then((res) => {
      ...
      state.optionValidityFlag =
        state.keyword ||
        (typeof initValue === "object" ? !isEmpty(initValue) : initValue)
          ? false
          : true;
    })
    .finally(() => {
