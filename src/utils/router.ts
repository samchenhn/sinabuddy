    ...
    if (isNavigationFailure(failure, NavigationFailureType.aborted)) {
      ElNotification({
        message: i18n.global.t(
          "utils.Navigation failed, navigation guard intercepted!",
        ),
        type: "error",
      });
      ...
      ElNotification({
        message: i18n.global.t(
          "utils.Navigation failed, it is at the navigation target position!",
        ),
        type: "warning",
      });
    }
...
export const onClickMenu = (menu: RouteRecordRaw) => {
  switch (menu.meta?.menu_type) {
    ...
    default:
      ElNotification({
        message: i18n.global.t(
          "utils.Navigation failed, the menu type is unrecognized!",
        ),
        type: "error",
      });
      break;
...
export const handleFrontendRoute = (routes: any, menus: any) => {
  ...
  if (routes.length) {
    ...
    memberCenter.mergeNavUserMenus(
      handleMenuRule(routes, "/", ["nav_user_menu"]),
    );
  }
  if (menus.length && isEmpty(memberCenter.state.viewRoutes)) {
    ...
    const menuMemberCenterBaseRoute =
      (memberCenterBaseRoute.path as string) + "/";
    memberCenter.mergeAuthNode(
      handleAuthNode(menus, menuMemberCenterBaseRoute),
    );

    memberCenter.mergeNavUserMenus(
      handleMenuRule(menus, "/", ["nav_user_menu"]),
    );
    memberCenter.setShowHeadline(menus.length > 1);
    memberCenter.setViewRoutes(
      handleMenuRule(menus, menuMemberCenterBaseRoute),
    );
  }
};
  ...
  for (const key in routes) {
    ...
    if (
      ...
      ((routes[key].menu_type == "tab" && !routes[key].component) ||
        (["link", "iframe"].includes(routes[key].menu_type) &&
          !routes[key].url))
    ) {
      continue;
    }
