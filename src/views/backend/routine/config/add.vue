      ...
      <div
        ...
        :style="
          config.layout.shrink
            ? ''
            : 'width: calc(100% - ' + state.labelWidth / 2 + 'px)'
        "
      >
        <el-form
          ref="formRef"
          ...
          <FormItem
            ...
            :placeholder="
              t('Please select field', {
                field: t('routine.config.Variable group'),
              })
            "
          />
...
const rules = reactive<FormRules>({
  ...
  name: [
    buildValidatorData({
      name: "required",
      title: t("routine.config.Variable name"),
    }),
    buildValidatorData({
      name: "varName",
      message: t("Please enter the correct field", {
        field: t("routine.config.Variable name"),
      }),
    }),
  ],
  title: [
    buildValidatorData({
      name: "required",
      title: t("routine.config.Variable title"),
    }),
  ],
  type: [
    buildValidatorData({
      ...
      trigger: "change",
      message: t("Please select field", {
        field: t("routine.config.Variable type"),
      }),
    }),
  ],
  weigh: [
    buildValidatorData({ name: "integer", title: t("routine.config.number") }),
  ],
});
