...
export const getCellValue = (
  ...
  // 若无值，尝试取默认值
  if (
    [undefined, null, ""].includes(cellValue) &&
  ...
  if (field.renderFormatter && typeof field.renderFormatter == "function") {
    ...
    console.warn(
      "baTable.table.column.renderFormatter 即将废弃，请直接使用兼容 el-table 的 baTable.table.column.formatter 代替",
    );
  }
 ...
 */
export const findIndexRow = (
  data: TableRow[],
  findIdx: number,
  keyIndex: number | TableRow = -1,
): number | TableRow => {
