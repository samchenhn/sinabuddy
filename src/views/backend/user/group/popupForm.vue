        ...
        >
          <el-form-item prop="name" :label="t('user.group.Group name')">
            <el-input
              ...
              type="string"
              :placeholder="
                t('Please input field', { field: t('user.group.Group name') })
              "
            ></el-input>
          </el-form-item>
...
const rules: Partial<Record<string, FormItemRule[]>> = reactive({
  name: [
    buildValidatorData({ name: "required", title: t("user.group.Group name") }),
  ],
  auth: [
    {
      required: true,
      validator: (rule: any, val: string, callback: Function) => {
        ...
        if (ids.length <= 0) {
          return callback(
            new Error(
              t("Please select field", { field: t("user.group.jurisdiction") }),
            ),
          );
        }
        return callback();
