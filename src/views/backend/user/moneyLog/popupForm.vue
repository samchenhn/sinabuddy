        ...
        >
          ...
          <el-form-item prop="money" :label="t('user.moneyLog.Change amount')">
            <el-input
              ...
              type="number"
              :placeholder="
                t('user.moneyLog.Please enter the balance change amount')
              "
            ></el-input>
          </el-form-item>
          ...
          <el-form-item prop="memo" :label="t('user.moneyLog.remarks')">
            <el-input
              ...
              type="textarea"
              :placeholder="
                t('user.moneyLog.Please enter change remarks / description')
              "
            ></el-input>
          </el-form-item>
        </el-form>
...
const rules: Partial<Record<string, FormItemRule[]>> = reactive({
  ...
  money: [
    buildValidatorData({
      name: "required",
      title: t("user.moneyLog.Change amount"),
    }),
    {
      validator: (rule: any, val: string, callback: Function) => {
        if (!val || parseFloat(val) == 0) {
          return callback(
            new Error(
              t("Please enter the correct field", {
                field: t("user.moneyLog.Change amount"),
              }),
            ),
          );
        }
  ...
  memo: [
    buildValidatorData({ name: "required", title: t("user.moneyLog.remarks") }),
  ],
});
...
const getAdd = () => {
  if (
    ...
    parseInt(baTable.form.items!.user_id) <= 0
  ) {
    return;
  }
...
const changeMoney = (value: string) => {
  ...
  state.after = parseFloat(
    (parseFloat(state.userInfo.money) + newValue).toFixed(2),
  );
};
