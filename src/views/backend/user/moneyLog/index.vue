    ...
    <TableHeader
      ...
      :quick-search-placeholder="
        t('Quick search placeholder', {
          fields:
            t('user.moneyLog.User name') +
            '/' +
            t('user.moneyLog.User nickname'),
        })
      "
    >
      <el-button
        v-if="!isEmpty(state.userInfo)"
        v-blur
...
const baTable = new baTableClass(
  new baTableApi(url),
  {
    column: [
      ...
      {
        label: t("user.moneyLog.Change balance"),
        prop: "money",
        align: "center",
        operator: "RANGE",
        sortable: "custom",
      ...
      {
        label: t("user.moneyLog.Before change"),
        prop: "before",
        align: "center",
        operator: "RANGE",
        sortable: "custom",
      ...
      {
        label: t("user.moneyLog.After change"),
        prop: "after",
        align: "center",
        operator: "RANGE",
        sortable: "custom",
...
baTable.before.onTableAction = ({ event }) => {
  ...
  if (event === "com-search") {
    ...
    for (const key in baTable.table.filter!.search) {
      if (
        ["money", "before", "after"].includes(
          baTable.table.filter!.search[key].field,
        )
      ) {
        ...
        for (const k in val) {
          newVal.push(
            isNaN(parseFloat(val[k])) ? "" : parseFloat(val[k]) * 100,
          );
        }
