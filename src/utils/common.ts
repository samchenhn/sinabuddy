...
export function auth(node: string | { name: string; subNodeName?: string }) {
  ...
  if (typeof node === "string") {
    ...
    if (store.state.authNode.has(path)) {
      ...
      if (
        store.state.authNode.get(path)!.some((v: string) => v == subNodeName)
      ) {
        return true;
      }
    }
    ...
    if (
      !node.subNodeName ||
      store.state.authNode.get(node.name)?.includes(node.subNodeName)
    )
      return true;
  }
  return false;
...
export const __ = (
  ...
  if (isAdminApp()) {
    langPath = path.slice(
      path.indexOf(adminBaseRoutePath) + adminBaseRoutePath.length,
    );
...
export const checkFileMimetype = (fileName: string, fileType: string) => {
  ...
  const fileSuffix = fileName
    ...
    .toLowerCase();
  if (
    allowedSuffixes.includes(fileSuffix) ||
    allowedSuffixes.includes("." + fileSuffix)
  ) {
    return true;
  }
