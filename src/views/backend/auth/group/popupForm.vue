        ...
        >
          ...
          <el-form-item prop="name" :label="t('auth.group.Group name')">
            <el-input
              ...
              type="string"
              :placeholder="
                t('Please input field', { field: t('auth.group.Group name') })
              "
            ></el-input>
          </el-form-item>
...
const rules: Partial<Record<string, FormItemRule[]>> = reactive({
  name: [
    buildValidatorData({ name: "required", title: t("auth.group.Group name") }),
  ],
  auth: [
    {
      required: true,
      validator: (rule: any, val: string, callback: Function) => {
        ...
        if (ids.length <= 0) {
          return callback(
            new Error(
              t("Please select field", { field: t("auth.group.jurisdiction") }),
            ),
          );
        }
        return callback();
  ...
  pid: [
    {
      validator: (rule: any, val: string, callback: Function) => {
        ...
        if (parseInt(val) == parseInt(baTable.form.items!.id)) {
          return callback(
            new Error(
              t("auth.group.The parent group cannot be the group itself"),
            ),
          );
        }
        return callback();
