...
<template>
  <div>
    <el-dialog
      v-model="model"
      class="ba-account-dialog"
      width="25%"
      :title="t('layouts.Member information')"
    >
      <template v-if="baAccount.token">
        <div v-loading="state.loading" class="userinfo">
          ...
          <div class="userinfo-buttons">
            <a
              href="https://uni.buildadmin.com/user"
              target="_blank"
            ...
            <el-button
              @click="baAccount.logout()"
              v-blur
              size="default"
              type="warning"
              >{{ $t("layouts.Logout") }}</el-button
            >
          </div>
        </div>
      </template>
        ...
        <div class="ba-login">
          ...
          >
            <FormItem
              ...
              prop="username"
              :placeholder="
                t('layouts.Please enter buildadmin account name or email')
              "
              :input-attr="{
                size: 'large',
              }"
            ...
            <FormItem
              ...
              prop="password"
              :placeholder="
                t('layouts.Please enter the buildadmin account password')
              "
              :input-attr="{
                size: 'large',
              }"
...
const onBaAccountSubmitPre = () => {
  baAccountFormRef.value?.validate((valid) => {
    if (valid) {
      clickCaptcha(
        state.user.captchaId,
        (captchaInfo: string) => onBaAccountSubmit(captchaInfo),
        { apiBaseURL: siteConfig.apiUrl },
      );
    }
  });
};
...
const baAccountFormRules: Partial<Record<string, FormItemRule[]>> = reactive({
  username: [
    buildValidatorData({ name: "required", title: t("layouts.Username") }),
  ],
  password: [
    buildValidatorData({ name: "required", title: t("layouts.Password") }),
