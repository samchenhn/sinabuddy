...
export default {
  ...
  "Database connection help":
    "您可以在 config/database.php 内配置多个数据库连接，然后在此处选择它，留空将使用默认连接配置",
  layouts: {
  ...
  terminal: {
    ...
    "NPM package manager tip":
      "选择一个可用的包管理器，用于 WEB 终端中 npm install 等命令的执行",
    "Clear successful task": "清理成功任务",
    "Clear successful task tip":
      "开始一个新任务时，自动清理列表中已经成功的任务",
  ...
  vite: {
    ...
    "Reload hot server tips 3":
      "热更新暂停不影响已经加载好的功能，您可以继续操作，并在一切就绪后再点击重新启动热更新服务。",
  },
};
