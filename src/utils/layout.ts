...
export function mainHeight(extra = 0): CSSProperties {
  ...
  if (isAdminApp()) {
    ...
    if (!navTabs.state.tabFullScreen) {
      height +=
        adminLayoutHeaderBarHeight[
          config.layout.layoutMode as keyof typeof adminLayoutHeaderBarHeight
        ];
    }
  } else {
    const memberCenter = useMemberCenter();
    height +=
      userLayoutHeaderBarHeight[
        memberCenter.state.layoutMode as keyof typeof userLayoutHeaderBarHeight
      ];
  }
  return {
