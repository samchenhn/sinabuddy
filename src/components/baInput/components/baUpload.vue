  ...
  <div class="w100">
    <el-upload
      ...
      :class="[
        ...
        state.attrs.limit &&
        state.fileList.length >= state.attrs.limit
          ? 'hide-image-plus'
          : '',
      ]"
    ...
    >
      <template v-if="!$slots.default" #default>
        <template v-if="type == 'image' || type == 'images'">
          <div
            v-if="!hideSelectFile"
            @click.stop="showSelectFile()"
            class="ba-upload-select-image"
          >
            {{ $t("utils.choice") }}
          </div>
          <Icon
            class="ba-upload-icon"
            name="el-icon-Plus"
            size="30"
            color="#c0c4cc"
          />
    ...
    <SelectFile
      v-model="state.selectFile.show"
      v-bind="state.selectFile"
      @choice="onChoice"
    />
  </div>
</template>
  ...
  fileUpload(fd, { uuid: uuid() }, props.forceLocal, {
    onUploadProgress: (evt: AxiosProgressEvent) => {
      ...
      if (
        evt.total &&
        evt.total > 0 &&
        ["ready", "uploading"].includes(file.status!)
      ) {
...
const initSort = () => {
  ...
  nextTick(() => {
    ...
    let uploadItemEl = uploadListEl.getElementsByClassName(
...
const triggerEvent = (name: string, args: any[]) => {
  ...
  if (events) {
    for (const key in events) {
      ...
      if (
        typeof state.events[events[key]] === "function" &&
        state.events[events[key]](...args) === false
      )
        return false;
    }
  }
...
onMounted(() => {
  ...
  if (!isEmpty(props.attr)) {
    ...
    console.warn(
      "图片/文件上传组件的 props.attr 已经弃用，并将于 v2.2.0 版本彻底删除，请将 props.attr 的部分直接作为 props 传递！",
    );
  }
...
const getAllUrls = (returnType: string = state.defaultReturnType) => {
  ...
  for (const key in state.fileList) {
    if (state.fileList[key].serverUrl)
 ...
 */
const onChange = (
  file: string | string[] | UploadFileExt,
  files: UploadFileExt[],
) => {
  initSort();
