      ...
      <div
        ...
        :style="
          config.layout.shrink
            ? ''
            : 'width: calc(100% - ' + baTable.form.labelWidth! / 2 + 'px)'
        "
      >
        <el-form
          ref="formRef"
        ...
        >
          ...
          <el-form-item :label="t('auth.rule.Rule type')">
            <el-radio-group v-model="baTable.form.items!.type">
              <el-radio class="ba-el-radio" value="route" :border="true">{{
                t("user.rule.Normal routing")
              }}</el-radio>
              <el-radio class="ba-el-radio" value="menu_dir" :border="true">{{
                t("user.rule.Member center menu contents")
              }}</el-radio>
              <el-radio class="ba-el-radio" value="menu" :border="true">{{
                t("user.rule.Member center menu items")
              }}</el-radio>
              <el-radio class="ba-el-radio" value="nav" :border="true">{{
                t("user.rule.Top bar menu items")
              }}</el-radio>
              <el-radio class="ba-el-radio" value="button" :border="true">{{
                t("user.rule.Page button")
              }}</el-radio>
              <el-radio
                class="ba-el-radio"
                value="nav_user_menu"
                :border="true"
                >{{ t("user.rule.Top bar user dropdown") }}</el-radio
              >
            </el-radio-group>
            <div class="block-help">
              {{ t("user.rule.Type " + baTable.form.items!.type + " tips") }}
            </div>
          </el-form-item>
          <el-form-item prop="title" :label="t('auth.rule.Rule title')">
            <el-input
              ...
              type="string"
              :placeholder="
                t('Please input field', { field: t('auth.rule.Rule title') })
              "
            ></el-input>
          </el-form-item>
          <el-form-item prop="name" :label="t('auth.rule.Rule name')">
            <el-input
              ...
              type="string"
              :placeholder="t('user.rule.English name')"
            ></el-input>
            <div class="block-help">
              {{
                t(
          ...
          <el-form-item
            ...
            prop="path"
            :label="t('auth.rule.Routing path')"
          >
            <el-input
              v-model="baTable.form.items!.path"
              type="string"
              :placeholder="t('user.rule.Web side routing path')"
            ></el-input>
          </el-form-item>
          ...
          <FormItem
            v-if="
              !['menu_dir', 'button', 'route'].includes(
            ...
            :input-attr="{
              border: true,
              content: {
                tab: t('auth.rule.Menu type tab'),
                link: t('auth.rule.Menu type link (offsite)'),
                iframe: 'Iframe',
              },
            }"
          />
          ...
          >
            <el-input
              ...
              :placeholder="
                t(
                  'auth.rule.Please enter the URL address of the link or iframe',
                )
              "
            ></el-input>
          </el-form-item>
          ...
          <el-form-item
            v-if="
              ...
              (!['menu_dir', 'button'].includes(baTable.form.items!.type) &&
                baTable.form.items!.menu_type == 'tab')
            "
          ...
          >
            <el-input
              ...
              :placeholder="
                t(
                  'user.rule.For example, if you add account/overview as a route only',
                )
              "
            ></el-input>
            ...
            <el-select
              ...
              :placeholder="
                t('Please select field', {
                  field: t('auth.rule.Extended properties'),
                })
              "
            >
              <el-option :label="t('auth.rule.none')" value="none"></el-option>
              <el-option
                :label="t('auth.rule.Add as route only')"
                value="add_rules_only"
              ></el-option>
              <el-option
                :label="t('auth.rule.Add as menu only')"
                value="add_menu_only"
              ></el-option>
            </el-select>
            <div class="block-help">
              {{
                t(
                  "user.rule.Web side component path, please start with /src, such as: /src/views/frontend/index",
                )
              }}
            </div>
          </el-form-item>
          <FormItem
            v-if="
              !['menu_dir', 'menu', 'nav_user_menu'].includes(
                baTable.form.items!.type,
              )
            "
            ...
            :input-attr="{
              border: true,
              content: {
                '0': t('user.rule.no_login_valid 0'),
                '1': t('user.rule.no_login_valid 1'),
              },
            }"
          ...
          <el-form-item :label="t('auth.rule.Rule comments')">
            <el-input
              ...
              :placeholder="
                t('Please input field', { field: t('auth.rule.Rule comments') })
              "
            ></el-input>
          </el-form-item>
          <el-form-item :label="t('auth.rule.Rule weight')">
            <el-input
              ...
              type="number"
              :placeholder="
                t('auth.rule.Please enter the weight of menu rule (sort by)')
              "
            ></el-input>
          </el-form-item>
          <FormItem
...
const rules: Partial<Record<string, FormItemRule[]>> = reactive({
  title: [
    buildValidatorData({ name: "required", title: t("auth.rule.Rule title") }),
  ],
  pid: [
    {
      validator: (rule: any, val: string, callback: Function) => {
        ...
        if (parseInt(val) == parseInt(baTable.form.items!.id)) {
          return callback(
            new Error(
              t("auth.rule.The superior menu rule cannot be the rule itself"),
            ),
          );
        }
        return callback();
  ...
  name: [
    buildValidatorData({ name: "required", title: t("auth.rule.Rule name") }),
  ],
  path: [
    buildValidatorData({
      name: "required",
      title: t("auth.rule.Routing path"),
    }),
  ],
  url: [
    buildValidatorData({
      name: "required",
      message: t("auth.rule.Link address"),
    }),
    buildValidatorData({
      name: "url",
      message: t("auth.rule.Please enter the correct URL"),
    }),
  ],
});
</script>
