<template>
  <div>
    <el-dialog
      v-model="state.dialog.buy"
      class="buy-dialog"
      :title="t('module.Confirm order info')"
      top="20vh"
      width="28%"
    >
      <div v-loading="state.loading.buy">
        <el-alert
          :title="t('module.Module installation warning')"
          type="error"
          :center="true"
          :closable="false"
        />
        <div v-if="!isEmpty(state.buy.info)" class="order-info">
          ...
          <div class="order-info-item">
            ...
            <span v-if="!state.buy.info.purchased" class="order-price">
              {{
                currency(
                  state.buy.info.amount,
                  state.buy.info.pay.money ? 1 : 0,
                )
              }}
            </span>
            <span v-else class="order-price">{{
