        ...
        <el-row justify="center">
          <el-col :span="16" :xs="24">
            <div v-if="memberCenter.state.open" class="login-box">
              ...
              >
                ...
                <el-form-item v-if="state.form.tab == 'register'">
                  <el-radio-group
                    size="large"
                    v-model="state.form.registerType"
                  >
                    <el-radio
                      class="register-verification-radio"
                      value="email"
                      :disabled="
                        !state.accountVerificationType.includes('email')
                      "
                      border
                    >
                    ...
                    <el-radio
                      ...
                      value="mobile"
                      :disabled="
                        !state.accountVerificationType.includes('mobile')
                      "
                      border
                    >
                      {{
                    ...
                    :placeholder="
                      state.form.tab == 'register'
                        ? t('Please input field', {
                            field: t('user.login.User name'),
                          })
                        : t('Please input field', {
                            field: t('user.login.account'),
                          })
                    "
                    :clearable="true"
                    size="large"
                ...
                <el-form-item
                  v-if="
                    state.form.tab == 'register' &&
                    state.form.registerType == 'mobile'
                  "
                  prop="mobile"
                >
                  <el-input
                    ...
                    :placeholder="
                      t('Please input field', { field: t('user.login.mobile') })
                    "
                    :clearable="true"
                    size="large"
                  >
                ...
                >
                  <el-input
                    ...
                    :placeholder="
                      t('Please input field', { field: t('user.login.email') })
                    "
                    :clearable="true"
                    size="large"
                  >
                  ...
                  <el-row class="w100">
                    <el-col :span="16">
                      <el-input
                        ...
                        :placeholder="
                          t('Please input field', {
                            field: t('user.login.Verification Code'),
                          })
                        "
                        autocomplete="off"
                      >
                ...
                <div v-if="state.form.tab != 'register'" class="form-footer">
                  <el-checkbox
                    ...
                    :label="t('user.login.Remember me')"
                    size="default"
                  ></el-checkbox>
                  <div
            ...
            <el-alert
              v-else
              :center="true"
              :title="$t('Member center disabled')"
              type="error"
            />
          </el-col>
        </el-row>
      ...
      <div class="retrieve-password-form">
        ...
        >
          ...
          <el-form-item
            prop="account"
            :label="
              state.retrievePasswordForm.type == 'email'
                ? t('user.login.email')
                : t('user.login.mobile')
            "
          >
          ...
          <el-form-item
            prop="captcha"
            :label="t('user.login.Verification Code')"
          >
            <el-row class="w100">
              <el-col :span="16">
                <el-input
          ...
          <el-form-item prop="password" :label="t('user.login.New password')">
            <el-input
              ...
              :placeholder="
                t('Please input field', { field: t('user.login.New password') })
              "
              show-password
            >
              <template #prefix>
...
const rules: Partial<Record<string, FormItemRule[]>> = reactive({
  ...
  password: [
    buildValidatorData({ name: "required", title: t("user.login.password") }),
    buildValidatorData({ name: "password" }),
  ],
  mobile: [
    buildValidatorData({ name: "required", title: t("user.login.mobile") }),
    buildValidatorData({ name: "mobile" }),
  ],
  captcha: [
    buildValidatorData({
      name: "required",
      title: t("user.login.Verification Code"),
    }),
  ],
});
...
const retrieveRules: Partial<Record<string, FormItemRule[]>> = reactive({
  account: [
    buildValidatorData({
      name: "required",
      title: t("user.login.Account name"),
    }),
  ],
  captcha: [
    buildValidatorData({
...
const sendRegisterCaptchaPre = () => {
  ...
  formRef
    .value!.validateField([state.form.registerType, "username", "password"])
    .then((valid) => {
      if (!valid) return;
      clickCaptcha(state.form.captchaId, (captchaInfo: string) =>
...
const switchTab = (
  formRef: FormInstance | null | undefined = undefined,
  tab: "login" | "register",
) => {
  state.form.tab = tab;
...
onMounted(async () => {
  ...
  checkIn("get").then((res) => {
    ...
    state.retrievePasswordForm.type =
      res.data.accountVerificationType.length > 0
        ? res.data.accountVerificationType[0]
        : "";
  });
