<template>
  <div>
    <el-dialog
      v-model="state.dialog.goodsInfo"
      class="goods-info-dialog"
      :title="t('module.detailed information')"
      width="60%"
    >
      <el-scrollbar
        v-loading="state.loading.goodsInfo"
        :key="state.goodsInfo.uid"
        :height="500"
      >
        <div class="goods-info">
          <div class="goods-images">
            <el-carousel
              height="300"
              v-if="state.goodsInfo.images"
              indicator-position="outside"
            >
              <el-carousel-item
                class="goods-image-item"
                v-for="(image, idx) in state.goodsInfo.images"
                :key="idx"
              >
                <el-image
                  fit="contain"
                  :preview-src-list="state.goodsInfo.images"
          ...
          <div class="goods-basic">
            ...
            <div class="goods-tag">
              <el-tag
                v-for="(tag, idx) in state.goodsInfo.tags"
                :key="idx"
              ...
              <div class="basic-item-price">
                {{
                  typeof state.goodsInfo.currency_select != "undefined"
                    ? currency(
                        state.goodsInfo.present_price,
                        state.goodsInfo.currency_select,
                      )
                    : "-"
                }}
              </div>
            ...
            <div class="basic-item">
              ...
              <div class="basic-item-content">
                {{
                  state.goodsInfo.updatetime
                    ? timeFormat(state.goodsInfo.updatetime)
                    : "-"
                }}
              </div>
            </div>
            <div class="basic-item">
              ...
              <div class="basic-item-content">
                {{
                  state.goodsInfo.createtime
                    ? timeFormat(state.goodsInfo.createtime)
                    : "-"
                }}
              </div>
            </div>
            <div
              v-if="
                !installButtonState.stateSwitch.includes(state.goodsInfo.state)
              "
              class="basic-item"
            >
            ...
            <div class="basic-buttons">
              <el-dropdown
                v-if="
                  (!state.goodsInfo.purchased ||
                    installButtonState.InstallNow.includes(
              ...
              >
                <el-button class="basic-button-demo" type="primary">
                  <span class="basic-button-dropdown-span">{{
                    t("module.View demo")
                  }}</span>
              ...
              <el-button
                v-if="
                  (state.goodsInfo.state == moduleInstallState.UNINSTALLED &&
                    state.goodsInfo.purchased) ||
              ...
              <el-button
                v-if="
                  installButtonState.continueInstallation.includes(
                    state.goodsInfo.state,
                  )
                "
                @click="
              ...
              <el-button
                v-if="
                  installButtonState.alreadyInstalled.includes(
                    state.goodsInfo.state,
                  )
                "
                v-blur
              ...
              <el-button
                v-if="
                  state.goodsInfo.new_version &&
                  installButtonState.updateButton.includes(
                    state.goodsInfo.state,
                  )
                "
                @click="
                  onUpdate(state.goodsInfo.uid, state.goodsInfo.purchased)
                "
          ...
          <div
            v-if="!isEmpty(state.goodsInfo.developer)"
            class="goods-developer"
          >
            <div class="developer-header">
              <el-avatar :size="60" :src="state.goodsInfo.developer.avatar" />
              <div class="developer-name">
                <h3 class="developer-nickname">
                  {{ state.goodsInfo.developer.nickname }}
                </h3>
                <div class="developer-group">
                  {{
                    state.goodsInfo.developer.group
                      ? state.goodsInfo.developer.group
                      : "-"
                  }}
                </div>
            ...
            <div v-if="state.goodsInfo.qq" class="developer-contact">
              <h4 class="developer-info-title">
                {{ t("module.Contact developer") }}
              </h4>
            ...
            <div class="developer-recommend">
              ...
              <div
                v-if="state.goodsInfo.developer.goods.length > 0"
        ...
        <div
          class="goods-detail ba-markdown"
          v-html="state.goodsInfo.detail_editor"
        ></div>
        <div class="goods-version">
                ...
                <el-card class="version-card" shadow="hover">
                  <template #header>
                    <div class="version-card-header">
                      <h2>{{ version.title }}</h2>
                      <span class="version-short-describe">{{
                        version.short_describe
                      }}</span>
                    </div>
                  </template>
                  <div
          ...
          <div v-else class="empty-update-log">
            {{ $t("module.No detailed update log") }}
          </div>
        </div>
      </el-scrollbar>
...
const installButtonState = {
  ...
  updateButton: [
    moduleInstallState.WAIT_INSTALL,
    moduleInstallState.INSTALLED,
    moduleInstallState.DISABLE,
  ],
...
const onUpdate = (uid: string, order: number) => {
  ...
  if (!order) {
    ElMessageBox.confirm(
      t("module.No module purchase order was found within the expiration date"),
      t("Reminder"),
      {
  ...
  getInstallState(uid)
    .then((res) => {
      if (res.data.state == moduleInstallState.DISABLE) {
        ...
        ElMessageBox.confirm(
          t(
            "module.You need to disable this module before updating Do you want to disable it now?",
          ),
          t("Reminder"),
          {
