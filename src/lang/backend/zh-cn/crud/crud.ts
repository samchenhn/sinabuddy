export default {
  ...
  "Check model class":
    "请检查以上数据模型类中是否已经配置 protected $connection = '{connection}';",
  ...
  designChangeTips:
    "取消勾选后，则该项变动不会尝试同步至数据表（通常用于已经手动修改过表结构等情况）",
  tableReBuild: "删表重建",
  tableReBuildBlockHelp:
    "不调整表结构，直接删除已有数据表并重建，此举可以确保CRUD代码/记录与数据表结构一致",
  Yes: "是",
  No: "否",
  ...
  "Vite hot warning":
    "未找到 Vite 热更新服务，请在开发环境生成代码，或点击右上角的WEB终端重新发布",
  "Reset generate type attr":
    "字段生成类型已修改，是否将字段设计重置为新类型预设的方案？",
