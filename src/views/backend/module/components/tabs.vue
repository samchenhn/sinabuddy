<template>
  <div>
    ...
    >
      <el-tab-pane
        v-for="cat in state.table.category"
        :name="cat.id.toString()"
        :key="cat.id"
        :label="cat.name"
        class="store-tab-pane"
      >
        <template
          v-if="
            state.table.modules[state.table.params.activeTab] &&
            state.table.modules[state.table.params.activeTab].length > 0
          "
        >
                ...
                <div class="goods-footer">
                  ...
                  <div class="goods-data">
                    <span class="download-count">
                      <Icon name="fa fa-download" color="#c0c4cc" size="13" />
                      {{ item.downloads ? item.downloads : "-" }}
                    </span>
                    <span
                      v-if="item.state === moduleInstallState.UNINSTALLED"
                      class="goods-price"
                    >
                    ...
                    <div v-else class="goods-price">
                      <el-tag
                        effect="dark"
                        :type="
                          item.stateTag.type ? item.stateTag.type : 'primary'
                        "
                      >
                        {{ item.stateTag.text }}
                      </el-tag>
