<template>
  <div class="install-done">
    <div class="install-done-title">
      ...
      <span
        v-else-if="state.common.moduleState == moduleInstallState.DISABLE"
        >{{ t("module.Module is disabled") }}</span
      >
      <span
        v-else-if="
    ...
    <div class="install-tis-box">
      <div v-if="state.common.dependInstallState != 'none'" class="depend-box">
        <div
          class="depend-loading"
          v-if="state.common.dependInstallState == 'executing'"
          v-loading="true"
        ></div>
        <div class="depend-tis">
          <div v-if="state.common.dependInstallState == 'executing'">
            ...
            <span>
              {{
                t(
                  "module.The built-in terminal of the system is automatically installing these dependencies, please wait~",
                )
              }}
            </span>
            <span class="span-a" @click="showTerminal">{{
              t("module.View progress")
            }}</span>
          </div>
          <div
            v-if="state.common.dependInstallState == 'success'"
            class="color-green"
          >
            {{ t("module.Dependency installation completed~") }}
          </div>
          <div
            v-if="state.common.dependInstallState == 'fail'"
            class="exec-fail color-red"
          >
            ...
            <el-link
              target="_blank"
              type="primary"
              href="https://doc.buildadmin.com/guide/install/manualOperation.html"
            >
      ...
      <div
        v-else-if="state.common.moduleState == moduleInstallState.INSTALLED"
        class="depend-tis"
      >
    ...
    <div
      v-if="state.common.dependInstallState == 'fail'"
      class="install-tis-box text-align-center"
    >
    ...
    <div
      class="install-tis-box"
      v-if="
        hotUpdateState.dirtyFile &&
        state.common.moduleState != moduleInstallState.DISABLE
      "
    >
      <div class="install-form">
          ...
          <el-popover :width="360" placement="top">
            <div>
              <div class="el-popover__title">
                {{ t("vite.Reload hot server title") }}
              </div>
              <div class="reload-hot-server-content">
                <p>
                  ...
                  <span
                    >【{{
                      t(`vite.Close type ${hotUpdateState.closeType}`)
                    }}】</span
                  >
            ...
            <template #reference>
              <div class="block-help hot-server-tips">
                {{ t("module.detailed information") }}？
              </div>
            </template>
          </el-popover>
        </el-form-item>
...
const onConfirmDepend = () => {
  ElMessageBox.confirm(
    t(
      "module.Is the command that failed on the WEB terminal executed manually or in other ways successfully?",
    ),
    t("Reminder"),
    {
